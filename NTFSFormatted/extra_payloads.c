...
)
{
	...
	for (;; )
	{
		...
		{
			INT x = i % w, y = i / w; // me no likey :sob:
			prgbPixels[i].r += (255 * i);
		}
...
MessageBoxThread(VOID)
{
	...
	MessageBoxW(NULL, L"???", L"???", MB_ABORTRETRYIGNORE | MB_ICONWARNING);
...
)
{
	...
	if (Xorshift32() % 2)
	{
		BitBlt(hdc, rc.left, rc.top, w, h, hdc, rcOriginal.left, rcOriginal.top, (Xorshift32() % 2) ? SRCINVERT : SRCPAINT);
	}
	else
	{
		...
		StretchBlt(hdc, rcOriginal.left, rcOriginal.top, w, h, hdcDesktop, rcScrBounds.left, rcScrBounds.top,
			...
			(Xorshift32() % 2) ? SRCINVERT : SRCERASE);
	}
