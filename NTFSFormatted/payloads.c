...
)
{
	...
	{
		INT nFreq = (INT)(FastSine((FLOAT)t / 100.f) * 10.f + 800.f);
		FLOAT fSine = FastSine((FLOAT)t / 10.f) * (FLOAT)nSamplesPerSec;
		psSamples[t] = (SHORT)(TriangleWave(t, nFreq, (FLOAT)nSamplesPerSec * 8.f + fSine) * (FLOAT)SHRT_MAX * .1f) +
	...
	for (INT z = 0; z < nCubeRoot; z++)
	{
		for (INT y = 0; y < nCubeRoot; y++)
		{
			for (INT x = 0; x < nCubeRoot; x++)
			{
				...
				psSamples[nIndex] =
					(SHORT)(SquareWave(y + z * x, (nFreq % 800) + 2000, nSamplesPerSec) * (FLOAT)SHRT_MAX * .3f) +
					(SHORT)(SawtoothWave(x | z, (150 - (nFreq % 500) / 4) + 500, nSamplesPerSec) * (FLOAT)SHRT_MAX * .3f) +
...
)
{
	...
	for (INT t = 0; t < nSampleCount; t++)
	{
		INT nFreq = (INT)(FastSine((FLOAT)t / (100.f - t / (nSampleCount / 2000))) * 200.f + 100.f);
...
)
{
	...
	for (INT t = 0; t < nSampleCount * 2; t++)
	{
		BYTE bFreq = (BYTE)((t & ((t >> 18) + ((t >> 11) * t))) ^ t + (((t >> 8 | t) - (t >> 3 & t >> 2 | t >> 16)) & 128));
		((BYTE*)psSamples)[t] = bFreq;
	}
}
...
)
{
	...
	for (INT t = 0; t < nSampleCount; t++)
	{
		...
		psSamples[t] = (SHORT)(SawtoothWave(t, sRand, nSampleCount) * (FLOAT)SHRT_MAX * .1f)
			...
			(SHORT)(TriangleWave(t, 300, nSampleCount) * (FLOAT)SHRT_MAX);
	}
}
...
)
{
	...
	for (INT y = 0; y < nSquareRoot; y++)
	{
		for (INT x = 0; x < nSquareRoot; x++)
		{
			...
			INT nFreq = (INT)((FLOAT)(y | x) * FastSine((FLOAT)(y * x) / 100.f));
			psSamples[nIndex] =
...
)
{
	...
	for (INT t = 0; t < nSampleCount; t++)
	{
		psSamples[t] = (SHORT)(TriangleWave(__rdtsc() % 8, 100, nSampleCount) * (FLOAT)SHRT_MAX * .3f) |
...
)
{
	...
	for (INT t = 0; t < nSampleCount; t++)
	{
		psSamples[t] = (SHORT)(SawtoothWave(__rdtsc() % 1500, 500, nSampleCount) * (FLOAT)SHRT_MAX * .3f) ^
			((SHORT)(SawtoothWave(t % 10, t % 100, nSampleCount) * (FLOAT)SHRT_MAX * .1f) >> 8);
	}
}
...
)
{
	...
	for (INT y = 0; y < nSquareRoot; y++)
	{
		for (INT x = 0; x < nSquareRoot; x++)
		{
			...
			psSamples[nIndex] = (SHORT)(SineWave(y + x, (nFreq % 100) + 100, nSamplesPerSec) * (FLOAT)SHRT_MAX * .3f);
		}
	}
}
...
)
{
	...
	for (INT y = 0; y < nSquareRoot; y++)
	{
		for (INT x = 0; x < nSquareRoot; x++)
		{
			...
			INT nFreq = (INT)((FLOAT)(y ^ x) * exp(cosh(atanf((FLOAT)(y | x)) / 100.f)) * 2.f);
			psSamples[nIndex] = (SHORT)(SineWave(y - (x % (y + 100)), (nFreq % 100) + 500, nSamplesPerSec) * (FLOAT)SHRT_MAX * .3f);
		}
	}
}
	...
	for (INT z = 0; z < nCubeRoot; z++)
	{
		for (INT y = 0; y < nCubeRoot; y++)
		{
			for (INT x = 0; x < nCubeRoot; x++)
			{
				...
				INT nFreq = (INT)((FLOAT)(y & x) * logf((FLOAT)z / (FLOAT)nCubeRoot + (FLOAT)x + (FLOAT)nCounter * (FLOAT)y) * 2.f);
...
)
{
	...
	for (INT y = 0; y < h; y++)
	{
		for (INT x = 0; x < w; x++)
		{
			...
			rgbDst.g = ~prgbSrc[y * w + x].b / 32;
			rgbDst.b = ~prgbSrc[y * w + x].r / 32;
...
)
{
	...
	{
		hbrBall = CreateSolidBrush(RGB(Xorshift32() % 255, Xorshift32() % 255, Xorshift32() % 255));
...
)
{
	...
	FLOAT a = (FLOAT)t / 500.f;
	FLOAT b = FastSine(a) * _t;
	FLOAT c = FastCosine(a) * _t;
	FLOAT centerX = (FLOAT)w / 5;
	FLOAT centerY = (FLOAT)h / 5;
...
)
{
	...
	if (!(t % 4))
	{
		StretchBlt(hdcTemp, Xorshift32() % 3, Xorshift32() % 3, rcBounds.left, rcBounds.top, hdcTemp, Xorshift32() % 3, Xorshift32() % 3, rcBounds.bottom, rcBounds.right, NOTSRCCOPY);
	}
	else
	{
		RedrawWindow(NULL, NULL, NULL, RDW_ERASE | RDW_INVALIDATE | RDW_ALLCHILDREN);
...
)
{
	...
	for (INT y = 0; y < h; y++)
	{
		for (INT x = 0; x < w; x++)
		{
			u = x + (INT)a + (INT)(TriangleWave(y, 100, h) * 160.f);
			v = y + (INT)b;
			...
			hsl = RGBToHSL(rgbDst);
			hsl.h = (FLOAT)fmod((DOUBLE)hsl.h + (DOUBLE)(x * y) / 1000000.0 + 0.05, 1.0);
			hsl.s = 1.f;
...
)
{
	...
	for (INT y = 0; y < h; y++)
	{
		for (INT x = 0; x < w; x++)
		{
			...
			FLOAT _r = (FLOAT)rgbDst.b * f + (FLOAT)rgbSrc.r * (1.f - f);
			FLOAT _g = (FLOAT)rgbDst.g * f + (FLOAT)rgbSrc.g * (1.f - f);
			FLOAT _b = (FLOAT)rgbDst.b * f + (FLOAT)rgbSrc.r * (1.f - f);
			rgbDst.rgb = ((BYTE)_b | ((BYTE)_g << 8) | ((BYTE)_r << 16));
...
)
{
	...
	for (INT y = 0; y < h; y++)
	{
		for (INT x = 0; x < w; x++)
		{
			...
			if ((roundf(hsl.h * 10.f) / 10.f) != (roundf((FLOAT)((Xorshift32() + t) % 27) / 26.f * 10.f) / 10.f))
			{
...
)
{
	...
	for (INT y = 0; y < h; y++)
	{
		FLOAT a = TriangleWave(t * 8 + y, 10, h) * 10.f;
...
)
{
	...
	for (INT y = 0; y < h; y++)
	{
		...
		for (INT x = 0; x < w; x++)
		{
			...
			rgbDst = prgbSrc[v * w + u];
			rgbDst.rgb = ((rgbDst.r + (x + y)) | ((rgbDst.g + (x + y)) << 8) | ((rgbDst.r + (x & y)) << 16));
			prgbDst[y * w + x] = rgbDst;
		}
	}
}
...
)
{
	...
	for (INT y = 0; y < h; y++)
	{
		for (INT x = 0; x < w; x++)
		{
			FLOAT a = sinhf(atanf((FLOAT)(((t + x) * 32) & t))) * tanf((FLOAT)(t | 26)) * 32.f;
			FLOAT b = expf((FLOAT)atan((DOUBLE)t / sinh(10.0)) + (FLOAT)(x ^ y));
...
)
{
	...
	for (INT y = 0; y < h; y++)
	{
		for (INT x = 0; x < w; x++)
		{
			FLOAT a = (FLOAT)ldexp((DOUBLE)atanf((FLOAT)(((t + x) * 7) & t)), t + y) * (FLOAT)scalbn((DOUBLE)(t | 256), x & y * 24) * 32.f;
			...
			hsl.h = (FLOAT)fmod((DOUBLE)rgbDst.r / 25.0 + (DOUBLE)t / 128.0, 1.0);
			hsl.s = (FLOAT)rgbDst.g / 25.f;
			hsl.l = (FLOAT)rgbDst.b / 25.f;
			rgbDst = HSLToRGB(hsl);
			prgbDst[y * w + x] = rgbDst;
		}
	}
...
)
{
	...
	for (INT y = 0; y < h; y++)
	{
		FLOAT a = TriangleWave(t * 1 + y / 2, 2, w) * SquareWave(t * 8 + y / 2, 2, h) * 4.f;

		for (INT x = 0; x < w; x++)
		{
			FLOAT b = TriangleWave(t * 2 + x / 2, 2, w) * SquareWave(t * 8 + y / 2, 2, h) * 4.f;
...
)
{
	...
	for (INT y = 0; y < h; y++)
	{
		...
		for (INT x = 0; x < w; x++)
		{
			...
			rgbDst = prgbSrc[v * w + u];
			rgbDst.rgb += (COLORREF)(__rdtsc() & x & (__rdtsc() & 0b100000001000000010000000));
			prgbDst[y * w + x] = rgbDst;
		}
	}
}
...
)
{
	...
	HSLCOLOR hsl;
	hsl.h = (FLOAT)fmod((DOUBLE)t / 12.0, 1.0);
	hsl.s = 1.f;
	hsl.l = .5f;
...
)
{
	...
	{
		HSLCOLOR hsl;
		hsl.h = (FLOAT)fmod((DOUBLE)t / 36.0 + (DOUBLE)y / (DOUBLE)h * 2.f, 1.0);
		hsl.s = 1.f;
		hsl.l = .5f;
		COLORREF crRainbow = HSLToRGB(hsl).rgb;

		FLOAT a = FastSine((FLOAT)t / 16.f + (FLOAT)y / 32.f) * 8.f;
...
)
{
	...
	{
		rgbDst.rgb = (Xorshift32() % 2) * 0x010101;
